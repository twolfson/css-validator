...
function CssValidator(options) {
  ...
  // If there was a uri or text, ourselves (and the request) now
  if (options.uri || options.text) {
    form.pipe(req);
    req.end();
  } else {
  ...
  // DEV: We must return an emitter (not XmlToErrorStream) so we can return emit request based errors
