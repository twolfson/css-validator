...
/** @arg {?string} str Input string to check.
    @return {!boolean} True if input is undefined, null, or entirely whitespace. */
...
function ValidationStream(options) {
  ...
  if (isBlank(this.options.text)) {
    // The W3C service only understands nonempty CSS. Fork a dummy event to be acted on after the
    // client has yielded.
    setTimeout(this._emitEmptyCssEvents.bind(this), 0);
  } else {
...
extend(ValidationStream.prototype, {
  ...
  generateRequest: function () {
    ...
    Object.getOwnPropertyNames(options).forEach(function sendOption (key) {
      var val = options[key];
      if (val && key !== 'output' && key !== 'text' && key !== 'objectMode') {
        form.append(key, val);
      }
    });
