...
var DuplexEvents = [
  ...
  'end',
  'data',
  // Allow propagation of `error` and don't double list `close`
  // 'error',
  // 'close',
  'drain',
  'close',
  ...
  listenForResponse: function () {
    ...
    req.on('response', function (response) {
      ...
      fwd(validator, that, function forwardEvents (event, data) {
        ...
        if (DuplexEvents.indexOf(event) !== -1) {
          return false;
        }
      ...
      validator.on('readable', function () {
        validator.read();

        // When the validator is complete
        validator.on('end', function () {
          // TODO: Emit validation result as `data`
          // TODO: Push null
          that.emit('end');
        });
      });
